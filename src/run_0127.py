...
def explicit_l1_score_test(score, dev_pair, adj, device):
    gid1, gid2 = dev_pair.T
    pair_score = score[np.ix_(gid1.tolist(), (gid2 - score.shape[0]).tolist())]
    ...
    new_adj_1 = new_adj_1 / (np.linalg.norm(new_adj_1, axis=-1, keepdims=True) + 1e-5)
    new_adj_2 = new_adj_2 / (np.linalg.norm(new_adj_2, axis=-1, keepdims=True) + 1e-5)
    pair_score = pair_score.T + new_adj_2 * pair_score.T * new_adj_1.T
...
class RUN():
    ...
    def load_dataset(self):
        ...
        for es_path in es_path_list:
            ...
            # explicit_l1_score_test(moda_np, self.dev_pair, self.adj, self.device)
            self.s_num, self.t_num = moda_np.shape
            self.side_modalities[es_path.split('/')[-1].split('.')[0]] = moda_np
            self.modal_num += 1
        # explicit_l1_score_test(sum(all_moda), self.dev_pair, self.adj, self.device)
        # exit(0)
