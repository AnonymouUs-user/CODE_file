...
import argparse
from model import ST_Encoder_Module, Loss_Module, Weight_train, Weight_train_2
import numpy as np
import torch
import gc
import random
...
class RUN():
    ...
    def draw(self):
        ...
        plt.plot(self.epoch_list, self.st_score, label='st_score')
        max_st_score = max(self.st_score)
        max_st_index = self.st_score.index(max_st_score)
        max_st_epoch = self.epoch_list[max_st_index]
        plt.text(max_st_epoch, max_st_score, f'{max_st_score:.4f}', fontsize=8, ha='center', va='bottom')

        # for x, y in zip(self.epoch_list, self.st_score):
        #     plt.text(x, y, f'{y:.4f}', fontsize=8, ha='center', va='bottom')

        plt.plot(self.epoch_list, self.all_score, label='all_score')
        max_all_score = max(self.all_score)
        max_all_index = self.all_score.index(max_all_score)
        max_all_epoch = self.epoch_list[max_all_index]
        plt.text(max_all_epoch, max_all_score, f'{max_all_score:.4f}', fontsize=8, ha='center', va='bottom')
        plt.title('../results/{}_rate{}_{}_sig{}_weight{}_start{}'.format(
            self.args.dataset, self.args.ratio, current_date, str(self.sig), str(self.args.weight), str(self.args.epoch_weight)))

        # for x, y in zip(self.epoch_list, self.all_score):
        #     plt.text(x, y, f'{y:.4f}', fontsize=8, ha='center', va='bottom')
        plt.legend()
        plt.savefig('../results/{}_rate{}_{}_sig{}_{}.png'.format(
            self.args.dataset, self.args.ratio, current_date, str(self.sig), str(self.args.weight)
        ))
        # plt.show()
    ...
    def load_dataset(self):
        ...
        # side_modalities 中加载所有entity对应的模态相关分数
        ...
        for es_path in es_path_list:
            att_name = self.encoder_type + '_attr'
            name_name = self.encoder_type + '_name'
            # if 'Image' in es_path:
            #     continue
            if 'attr' in es_path:
                # continue
                if att_name in es_path:
                    print('')
                else:
                    continue
            if 'name' in es_path:
                # continue
                if name_name in es_path:
                    print('')
                else:
                    continue
            print(es_path)
    ...
    def init_model(self):
        ...
        # self.weight_model = Weight_train(N_view=self.modal_num, device=self.device)
        ...
        for epoch in range(train_epoch):
            ...
            for i in range(0, len(self.train_pair_confidence), batch_size):
                ...
                if epoch > self.args.epoch_weight:
                    ...
                    # 联合loss
                    # loss = loss + cos_loss * self.weight
                    # loss不进行联合
                    # weight_loss += cos_loss
    ...
    def sinkhorn_test(self, sourceVec, targetVec, device, len_dev, gid1, gid2):
        ...
        count = 0
        # gid1_ = gid1.detach().numpy()
        # gid2_ = gid2.detach().numpy()
        if self.exp_mod:
        ...
        # fuse_data = [weight_norm_[idx] * np.linalg.norm(sim_mat_list[idx]) for idx in range(len(sim_mat_list))]
        # sim_mat = sum(fuse_data) / len(sim_mat_list)
