...
import argparse
from model import ST_Encoder_Module, Loss_Module, VAE_ST_Encoder_Module
import numpy as np
import torch
import gc
import random
...
class RUN():
    def __init__(self):
        ...
        self.loss_rate = self.args.loss_rate
        # self.csp = 1
        # self.csp = 2
    ...
    def parse_options(parser):
        ...
        parser.add_argument('loss_rate', type=float, help='which loss ratio', default='0,1')
    ...
    def init_model(self):
        ...
        if self.Imp == 'ST':
            if self.vae:
                self.depth = 2
                self.structure_encoder = VAE_ST_Encoder_Module(
                    node_hidden=128,
                    rel_hidden=128,
    ...
    def run(self):
        ...
        for epoch in range(train_epoch):
            ...
            for i in range(0, len(self.train_pair_confidence), batch_size):
                ...
                if self.vae:
                    vae_loss = feature_list[1]
                    loss += vae_loss * self.loss_rate
