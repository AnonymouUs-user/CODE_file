...
import argparse
from model import Loss_Module, ST_Encoder_Module_hyper, Aug_loss, Weight_train
import numpy as np
import torch
import gc
import random
...
class RUN():
    ...
    def parse_options(parser):
        ...
        parser.add_argument('weight', type=float, help='which hyper_weight', default='0,1')
    ...
    def load_dataset(self):
        ...
        with open("../results/{}_rate{}_{}_sig{}.txt".format(self.args.dataset, self.args.ratio, current_date, str(self.sig)), 'a') as f:
        ...
        with open(
                "../accuracy/{}_rate{}_{}_sig{}_accuracy_csp{}.txt".format(self.args.dataset, self.args.ratio, current_date,
    ...
    def init_model(self):
        ...
        if self.exp_mod:
            self.Weight_model = Weight_train(N_view=4, device=self.device).to(self.device)
        else:
            self.Weight_model = Weight_train(N_view=2, device=self.device).to(self.device)
    ...
    def run(self):
        ...
        for epoch in range(train_epoch):
            ...
            for i in range(0, len(self.train_pair_confidence), batch_size):
                ...
                # loss_2 = self.loss_model(batch_pair, feature_list[1], weight=True)
                aug_loss = self.aug_loss(batch_pair, feature_list[0], feature_list[1])

                # 对weight进行训练
                index_a = batch_pair[:, 0].long()
                index_b = batch_pair[:, 1].long()
                # index_a = torch.LongTensor(batch_pair[:, 0])
                # index_b = torch.LongTensor(batch_pair[:, 1])
                st_sim_mat = self.sim_results(feature_list[0][index_a], feature_list[0][index_b])
                hyper_sim_mat = self.sim_results(feature_list[1][index_a], feature_list[1][index_b])

                if self.exp_mod:
                    all_score = []
                    all_score.append(st_sim_mat)
                    # score需要做全部数据的
                    for _, score in self.side_modalities.items():
                        all_score.append(torch.tensor(score[np.ix_(index_a.tolist(),index_b.tolist())]).to(self.device))
                        self.weight_list, cs_loss = self.Weight_model(all_score)
                        loss = loss + cs_loss * self.weight
                else:
                    all_score = [st_sim_mat, hyper_sim_mat]
                    self.weight_list, cs_loss = self.Weight_model(all_score)
                    loss = loss + cs_loss * self.weight
            ...
            if epoch % 2 == 0 and epoch > 0:
                ...
                with torch.no_grad():
                    ...
                    # 双曲+欧式同时决策
                    # hyper_out_feature = feature_list[1].cpu()
                    # del feature_list
                    # torch.cuda.empty_cache()
                    # hyper_out_feature = hyper_out_feature / (torch.linalg.norm(hyper_out_feature, dim=-1, keepdim=True) + 1e-5)
                    # hyper_Lvec = hyper_out_feature[index_a]
                    # hyper_Rvec = hyper_out_feature[index_b]
                    # hyper_Lvec = hyper_Lvec / (torch.linalg.norm(hyper_Lvec, dim=-1, keepdim=True) + 1e-5)
                    # hyper_Rvec = hyper_Rvec / (torch.linalg.norm(hyper_Rvec, dim=-1, keepdim=True) + 1e-5)
                    # result = self.sinkhorn_ST_hyper(Lvec, Rvec, hyper_Lvec, hyper_Rvec, device=self.device,
                    #                                         len_dev=len(self.dev_pair))
                    ...
                    if hits0 > max_hit0:
                        max_hit0 = hits0
                        with open(
                                "../results/{}_rate{}_{}_sig{}.txt".format(self.args.dataset, self.args.ratio,
                                                                           current_date,
                                                                           str(self.sig)), 'a') as f:
                            f.write(str(epoch) + ':\n')
                            f.write(str(result) + '\n')
            ...
            if epoch >= 2 and epoch % 2 == 0 and self.semi_pred:
                self.structure_encoder.eval()
                time7 = time.time()
                with torch.no_grad():
                    ...
                    # hyper_Lvec = hyper_out_feature[gid1]
                    # hyper_Rvec = hyper_out_feature[gid2]
                    # hyper_Lvec = hyper_Lvec / (torch.linalg.norm(hyper_Lvec, dim=-1, keepdim=True) + 1e-5)
                    # hyper_Rvec = hyper_Rvec / (torch.linalg.norm(hyper_Rvec, dim=-1, keepdim=True) + 1e-5)
                    # hyper_scores = self.sim_results(hyper_Lvec, hyper_Rvec)
                    # new_pair.update(self.pred_pair_confidence(hyper_scores))
                    ...
                    right_pair_num, pair_accuracy = self.pair_accuacy(self.train_pair_confidence.cpu().numpy()[:, :2],
                                                                      self.pair_gold)
                    with open(
                            "../accuracy/{}_rate{}_{}_sig{}_accuracy_csp{}.txt".format(self.args.dataset,
    ...
    def sinkhorn_test(self, sourceVec, targetVec, device, len_dev):
        # sim_mat = self.sim_results(sourceVec, targetVec)
        if self.exp_mod:
            sim_mat = self.sim_results(sourceVec, targetVec) * self.weight_list[0].detach().cpu()
        else:
            sim_mat = self.sim_results(sourceVec, targetVec)
        count = 1
        if self.exp_mod:
            for _, side_score in self.side_modalities.items():
                ...
                sim_mat += torch.Tensor(side_score) * self.weight_list[count].detach().cpu()
                count = count + 1
        print(count)
        sim_mat = sim_mat / (len(self.side_modalities.items())+1)
    ...
    def sinkhorn_ST_hyper(self, st_sourceVec, st_targetVec, hyper_sourceVec, hyper_targetVec, device, len_dev):
        st_sim_mat = self.sim_results(st_sourceVec, st_targetVec)
        hyper_sim_mat = self.sim_results(hyper_sourceVec, hyper_targetVec)
        # sim_mat = st_sim_mat + hyper_sim_mat
        sim_mat = self.weight_list[0].detach().cpu() * st_sim_mat + self.weight_list[1].detach().cpu() * hyper_sim_mat
        count = 0
        if self.exp_mod:
            for _, side_score in self.side_modalities.items():
                sim_mat += torch.Tensor(side_score)
